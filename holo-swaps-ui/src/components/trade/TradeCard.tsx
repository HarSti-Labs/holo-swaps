...
export function TradeCard({ trade, className }: TradeCardProps) {
  ...
  return (
    <Link href={`/trades/${trade.id}`}>
      <div
        className={cn(
          "bg-slate-900 border border-slate-700 rounded-xl p-5 shadow-md shadow-black/40 card-hover cursor-pointer",
          className
        )}
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-base font-medium">@{counterparty.username}</p>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-base text-muted-foreground mb-1.5">You give</p>
            <div className="flex gap-1.5 flex-wrap">
              {myItems.slice(0, 3).map((item) => {
                ...
                return (
                  <span
                    key={item.id}
                    className="text-base bg-muted px-2 py-1 rounded-md font-medium"
                  >
                    {card?.name ?? "Unknown"}
                  </span>
                );
              ...
              {myItems.length > 3 && (
                <span className="text-base text-muted-foreground px-1 py-1">
          ...
          {/* Their cards */}
          <div className="flex-1">
            <p className="text-base text-muted-foreground mb-1.5">You receive</p>
            <div className="flex gap-1.5 flex-wrap">
