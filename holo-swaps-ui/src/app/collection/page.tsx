...
function MyCardsPageContent() {
  ...
  const filteredCollection = useMemo(() => {
    ...
    return collection.filter((item) => {
      ...
      if (q && !item.card.name.toLowerCase().includes(q) && !item.card.setName.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [collection, filterTradeOnly, filterConditions, filterFoil, filterFirstEdition, filterSet, collectionSearch]);
  ...
  const filteredWants = useMemo(() => {
    ...
    return wants.filter((w) => {
      ...
      if (q && !w.card.name.toLowerCase().includes(q) && !w.card.setName.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [wants, filterWantPriorities, filterWantCondition, filterWantSet, wantSearch]);
        ...
        <div className="mb-8 relative">
          ...
          <div className="relative z-10 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xl text-slate-300 hidden sm:flex items-center gap-3">
            ...
            {activeTab === "collection" ? (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 sm:px-6 sm:py-4 text-sm sm:text-base rounded-xl sm:rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold hover:from-blue-500 hover:to-purple-500 transition-all shadow-xl shadow-blue-500/30"
              >
        ...
        {activeTab === "collection" && (
          <>
            ...
            <div className="relative mb-4">
              ...
              />
              {collectionSearch && (
                <button onClick={() => setCollectionSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white transition-colors">
                  <X size={15} />
                </button>
              )}
            </div>
        ...
        {activeTab === "wants" && (
          <>
            ...
            <div className="relative mb-4">
              ...
              />
              {wantSearch && (
                <button onClick={() => setWantSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white transition-colors">
                  <X size={15} />
                </button>
              )}
            </div>
