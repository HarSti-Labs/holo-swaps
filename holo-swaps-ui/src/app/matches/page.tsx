    ...
    <div className="bg-card border border-border rounded-xl p-5 card-hover flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-right space-y-1">
          ...
          <div className="flex items-center justify-end gap-1 text-base text-muted-foreground">
            <TrendingUp size={11} />
            <span>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4 flex-1">
        <div>
          <p className="text-base text-muted-foreground font-medium mb-2">They have (you want)</p>
          <div className="space-y-1.5">
            {match.theyHave.slice(0, 3).map((card) => (
              <div key={card.collectionItemId} className="flex items-center justify-between bg-muted/50 rounded-lg px-2.5 py-1.5">
                <span className="text-base font-medium truncate">{card.cardName}</span>
                <span className="text-base text-muted-foreground ml-2 shrink-0">
                  {card.marketValue > 0 ? formatCurrency(card.marketValue) : "—"}
                </span>
              </div>
            ))}
            {match.theyHave.length > 3 && (
              <p className="text-base text-muted-foreground px-2.5">+{match.theyHave.length - 3} more</p>
            )}
          </div>
        </div>

        <div>
          <p className="text-base text-muted-foreground font-medium mb-2">You have (they want)</p>
          <div className="space-y-1.5">
            ...
            {match.youHave.length > 3 && (
              <p className="text-base text-muted-foreground px-2.5">+{match.youHave.length - 3} more</p>
            )}
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg bg-primary text-primary-foreground text-base font-medium hover:bg-primary/90 transition-colors mt-auto"
      >
  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-8">
      ...
      {Array.from({ length: totalPages }, (_, i) => i + 1)
        ...
        .map((item, i) =>
          item === "..." ? (
            <span key={`ellipsis-${i}`} className="px-2 text-muted-foreground text-base">…</span>
          ) : (
            <button
              key={item}
      ...
      <main className="container mx-auto px-4 py-8 max-w-5xl">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="relative">
              ...
              <select
                ...
                className="pl-8 pr-3 py-2 text-base rounded-lg border border-border bg-card appearance-none cursor-pointer hover:bg-muted transition-colors"
              >
                {SORT_OPTIONS.map((o) => (
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-2 px-3 py-2 rounded-lg border text-base font-medium transition-colors",
            ...
            >
              ...
              Filters
              {hasActiveFilters && (
                <span className="w-4 h-4 rounded-full bg-primary text-primary-foreground text-base font-bold flex items-center justify-center">
        ...
        {filtersOpen && (
          <div className="bg-card border border-border rounded-xl p-5 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-semibold">Filter Matches</h2>
              {hasActiveFilters && (
                <button onClick={resetFilters} className="flex items-center gap-1 text-base text-muted-foreground hover:text-foreground transition-colors">
                  <X size={12} /> Reset all
                </button>
              )}
            </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-card border border-border rounded-xl p-16 text-center">
            ...
            <p className="text-base text-muted-foreground mt-2 mb-4">Try loosening your filter criteria.</p>
            <button onClick={resetFilters} className="text-base text-primary hover:underline font-medium">
              Reset filters
            </button>
          </div>
        ) : (
